...
Под полосой кадров — секция **«Метрики»**: каждый покадровый ряд, который
есть в профиле (занятость главного потока и её разбивка на скрипты /
рендер / GC, ожидание GPU, блокировки, цена записи экрана, все
рендер-счётчики инструментированной сборки — шейпы, команды, бленды,
тесселяции, cacheAsBitmap…, память wasm, сетевые события), — отдельной
тонкой дорожкой на той же оси времени, каждая в своём масштабе. Ничего не
фильтруется и не ранжируется: провал FPS и то, что подскочило под ним,
лежат на одной вертикали, а выводы — тривиальные и неожиданные — делает
читатель. Наведение курсора показывает точные значения всех метрик в этой
точке времени, клик открывает разбор кадра; метрики, не менявшиеся за всю
сессию, перечислены строкой «без движения» (их плоскость — тоже факт).
