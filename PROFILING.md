...
- **Эпизоды** — главная точка входа: вьюер сам находит стабильный режим
  сессии, склеивает отклонения от него в эпизоды (спайк / просадка /
  дёргания; на FPS-графике они видны красными полосами) и для каждого
  ставит вердикт, куда ушло время кадра (CPU / ожидание GPU / блокировки
  вне событий), — а главное, показывает **чем медленные кадры отличаются
  от нормы**: каждая записанная покадровая метрика (рендер-счётчики,
  длительности по категориям, память, сеть) сравнивается с медианой
  соседних быстрых кадров, и в карточку попадают только значимые
  отклонения («шейпы ×2», «тесселяции появились: 100/кадр»). Порог
  эпизода подстраивается под сессию; равномерно неровный фон выносится
  отдельным псевдоэпизодом. Клик по эпизоду подсвечивает его на графиках,
  клик по «худшему кадру» открывает разбор кадра;
